...
export const getUserInterests = async (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des intérêts de l'utilisateur:",
      error,
    );
  ...
  try {
    ...
    if (!userId || isNaN(parseInt(userId, 10))) {
      throw new ValidationError(
        "userId est nécessaire et doit être un entier.",
      );
    }
    ...
    if (
      ...
      interests.length === 0 ||
      !Array.isArray(needsWithDuration) ||
      needsWithDuration.length === 0
    ) {
      throw new ValidationError(
        "Les tableaux d'intérêts et de besoins doivent être non vides.",
      );
    }
    ...
    for (const interest_id of interests) {
      ...
      if (isNaN(integerInterestId)) {
        throw new ValidationError(
          "Tous les interest_id doivent être des entiers.",
        );
      }
    ...
    console.error(
      "Erreur lors de la création de la relation utilisateur-intérêt:",
      error,
    );
...
export const updateUserInterests = async (req, res) => {
  ...
  try {
    ...
    const deleteResult = await UserInterest.destroy(
      { where: { user_id: userId } },
      { transaction: t },
    );
    ...
    const lastNeedWithDuration =
      needsWithDuration[needsWithDuration.length - 1];
    ...
    await t.rollback();
    console.error(
      "Erreur lors de la mise à jour des intérêts de l'utilisateur:",
      error,
    );
    res.status(500).json({ error: "Erreur du serveur" });
  }
...
export const deleteUserInterests = async (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la suppression des intérêts de l'utilisateur:",
      error,
    );
    res.status(500).json({ error: "Erreur du serveur" });
  }
