...
export const createInterest = async (req, res) => {
    try {
        // Validation de l'entrée, par exemple vérifier si le champ "nom" est fourni
        if (!req.body.name) {
        ...
        if (error instanceof ValidationError) {
            res.status(400).json({ error: error.message }); // Erreur de validation du client
        } else {
            res.status(500).json({ error: 'Erreur du serveur' }); // Autres erreurs du serveur
        }
    }
};
...
export const updateInterest = async (req, res) => {
    try {
        ...
        throw new ValidationError('Intérêt non trouvé'); // Utiliser l'erreur personnalisée
    } catch (error) {
