...
export const getSuggestionFromOpenAI = async (req, res) => {
  try {
    ...
    const enrichedPrompt = createOpenAIPromptWithContext(
      promptText,
      interests,
      needs,
    );
    const suggestionText = await openaiService.getGPT4Response(
      enrichedPrompt,
      userId,
    );
...
export const markSuggestionAsAddedToAgenda = async (req, res) => {
  try {
    ...
    if (updated) {
      const markSuggestionAsAddedToAgenda = await GPTSuggestion.findOne({
        where: { id: id },
      });
      return res
        .status(200)
        .json({ suggestion: markSuggestionAsAddedToAgenda });
    }
...
export const markSuggestionAsRemovedToAgenda = async (req, res) => {
  try {
    const { id } = req.params;
    const [updated] = await GPTSuggestion.update(
      { is_added_to_agenda: false },
      {
        where: { id: id },
      },
    );
    if (updated) {
      const markSuggestionAsRemovedToAgenda = await GPTSuggestion.findOne({
        where: { id: id },
      });
      return res
        .status(200)
        .json({ suggestion: markSuggestionAsRemovedToAgenda });
    }
    throw new ValidationError("Suggestion non trouvée");
  } catch (error) {
    console.error("Erreur lors du marquage de la suggestion:", error);
...
export const updateSuggestion = async (req, res) => {
  try {
    ...
    if (updated) {
      const updatedSuggestion = await GPTSuggestion.findOne({
        where: { id: id },
      });
